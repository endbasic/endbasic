...
/// A trait to perform an unchecked cast.
trait UncheckedFrom {
    type T;
    unsafe fn unchecked_from(value: Self::T) -> Self;
}

/// Implements an unsafe conversion function between two integer types.
///
...
macro_rules! impl_unchecked_cast {
    ( $name:ident, $from_ty:ty, $to_ty:ty, primitive ) => {
        pub(crate) unsafe fn $name(value: $from_ty) -> $to_ty {
...
mod tests {
    ...
    fn test_unchecked_u32_as_u8() {
        assert_eq!(10u8, unsafe { unchecked_u32_as_u8(10u32) });
    }

    #[test]
    fn test_unchecked_u32_as_u16() {
        assert_eq!(10u16, unsafe { unchecked_u32_as_u16(10u32) });
    }
