...
impl Command for InputCommand {
    ...
    async fn exec(&self, args: Vec<(Value, LineCol)>, machine: &mut Machine) -> CommandResult {
        ...
        // DO NOT SUBMIT: We have no tests for "input foo as bar".
        // DO NOT SUBMIT: The to_owneds/clone below are suspicious.
    ...
    async fn exec(&self, args: Vec<(Value, LineCol)>, machine: &mut Machine) -> CommandResult {
        ...
        fn get_coord(value: Value, pos: LineCol, name: &str) -> Result<u16, CallError> {
            ...
            match u16::try_from(i) {
                Ok(v) => Ok(v),
                Err(_) => {
                    return Err(CallError::ArgumentError(pos, format!("{} out of range", name)));
                }
            }
        }
...
impl Command for PrintCommand {
    ...
    async fn exec(&self, args: Vec<(Value, LineCol)>, machine: &mut Machine) -> CommandResult {
        ...
        while let Some((value, _pos)) = iter.next() {
            match value {
