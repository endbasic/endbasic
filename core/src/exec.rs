...
/// Provides controlled access to the environment passed to a callable.
pub struct Scope<'s> {
...
impl Drop for Scope<'_> {
    fn drop(&mut self) {
        self.context.value_stack.discard(self.nargs);
    }
}
...
impl<'s> Scope<'s> {
    ...
    pub fn pop_boolean_with_pos(&mut self) -> (bool, LineCol) {
        ...
        self.nargs -= 1;
        self.context.value_stack.pop_boolean_with_pos()
    }
    ...
    pub fn pop_double_with_pos(&mut self) -> (f64, LineCol) {
        debug_assert!(self.nargs > 0, "Not enough arguments in scope");
        self.nargs -= 1;
        self.context.value_stack.pop_double_with_pos()
    }
    ...
    pub fn pop_integer_with_pos(&mut self) -> (i32, LineCol) {
        debug_assert!(self.nargs > 0, "Not enough arguments in scope");
        self.nargs -= 1;
        self.context.value_stack.pop_integer_with_pos()
    }
    ...
    pub fn pop_string_with_pos(&mut self) -> (String, LineCol) {
        debug_assert!(self.nargs > 0, "Not enough arguments in scope");
        self.nargs -= 1;
        self.context.value_stack.pop_string_with_pos()
    }
    ...
    pub fn pop_varref_with_pos(&mut self) -> (SymbolKey, ExprType, LineCol) {
        debug_assert!(self.nargs > 0, "Not enough arguments in scope");
        self.nargs -= 1;
        self.context.value_stack.pop_varref_with_pos()
    }

    /// Sets the return value of this function to `value`.
    pub fn return_any(mut self, value: Value) -> Result<()> {
        self.drain();
        self.context.value_stack.push((value, self.fref_pos));
        Ok(())
    }
    ...
    pub fn return_boolean(mut self, value: bool) -> Result<()> {
        self.drain();
        self.context.value_stack.push((Value::Boolean(value), self.fref_pos));
        Ok(())
    }
    ...
    pub fn return_double(mut self, value: f64) -> Result<()> {
        self.drain();
        self.context.value_stack.push((Value::Double(value), self.fref_pos));
        Ok(())
    }
    ...
    pub fn return_integer(mut self, value: i32) -> Result<()> {
        self.drain();
        self.context.value_stack.push((Value::Integer(value), self.fref_pos));
        Ok(())
    }
...
impl Machine {
    ...
    fn assign_array(
        ...
        for _ in 0..nargs {
            let i = value_stack.pop_integer();
            ds.push(i);
        }

        let (value, _pos) = value_stack.pop().unwrap();
    ...
    fn dim_array(&mut self, value_stack: &mut Stack, span: &DimArrayISpan) -> Result<()> {
        let mut ds = Vec::with_capacity(span.dimensions);
        for _ in 0..span.dimensions {
            let (i, pos) = value_stack.pop_integer_with_pos();
            if i <= 0 {
    ...
    fn get_array_args(&self, value_stack: &mut Stack, nargs: usize) -> Result<Vec<i32>> {
    ...
    fn array_ref(
        ...
        let subscripts = self.get_array_args(value_stack, nargs)?;
        match self.symbols.load(key) {
            Some(Symbol::Array(array)) => {
                ...
                value_stack.push((value, vref_pos));
                Ok(())
            }
    ...
    fn exec_until_stop(&mut self, context: &mut Context) -> Result<InternalStopReason> {
        while context.pc < context.instrs.len() {
            let instr = &context.instrs[context.pc];
            match instr {
                ...
                Instruction::ArrayAssignment(name, vref_pos, nargs) => {
                    self.assign_array(&mut context.value_stack, name, *vref_pos, *nargs)?;
                    context.pc += 1;
                }

                Instruction::ArrayLoad(name, pos, nargs) => {
                    self.array_ref(&mut context.value_stack, name, *pos, *nargs)?;
                    context.pc += 1;
                }
                ...
                Instruction::DimArray(span) => {
                    self.dim_array(&mut context.value_stack, span)?;
                    context.pc += 1;
                }
    ...
    fn handle_error(&self, context: &mut Context, e: Error) -> Result<()> {
        if !e.is_catchable() {
            return Err(e);
        }

        context.last_error = Some(format!("{}", e));
        ...
        match context.err_handler {
            ...
            ErrorHandlerISpan::ResumeNext => {
                if context.instrs[context.pc].is_statement() {
                    ...
                    loop {
                        context.pc += 1;
                        if context.pc >= context.instrs.len() {
                            break;
