...
use console::{SdlConsole, SdlInput};
use endbasic_std::Signal;
use endbasic_std::console::{Console, ConsoleSpec, GraphicsConsole, Resolution};
use endbasic_std::gfx::lcd::BufferedLcd;
use endbasic_std::gfx::lcd::fonts::{FONT_VT220, Fonts};
...
pub fn setup(
    ...
    let (console, on_key_rx) = SdlConsole::new(resolution, font.glyph_size, signals_tx)?;
    let input = SdlInput(on_key_rx);
    let rasops = BufferedLcd::new(console, font);
    let console = GraphicsConsole::new(input, rasops, default_fg_color, default_bg_color)?;
